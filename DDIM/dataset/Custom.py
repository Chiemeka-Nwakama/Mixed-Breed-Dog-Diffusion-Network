...
def create_custom_dataset(data_path, batch_size, **kwargs):
    ...
    Args:
        ...
        kwargs:
            ...
            pin_memory (bool)
            num_workers (int)
            preview (bool): if True, show some cropped images
            preview_count (int): number of images to show
    """
    ...
    ann_root = Path(data_path).parent / "Annotations"
    ...
    preview_count = kwargs.get("preview_count", 3)
    print(f"\nPreviewing {preview_count} cropped images...\n")
    for i in range(min(preview_count, len(dataset))):
        img_path, label = dataset.samples[i]
        image = Image.open(img_path).convert(dataset.mode)

        bbox = dataset.load_bbox(img_path)
        if bbox is not None:
        ...
        plt.imshow(image)
        plt.title(f"Cropped • class={dataset.classes[label]} idx={i}")
        plt.axis("off")
        plt.show()
